...
| `run_tests` | `path?`, `filter?`, `runner?`, `timeout_seconds?` | `dotnet test` / `pytest` (under the project's `.venv` or `venv` when there is one) / `npm test` / `cargo test` / `go test` (auto-detected) |
