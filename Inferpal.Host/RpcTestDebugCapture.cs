...
/// <summary>
/// VS Code implementation of the <c>/tdd</c> capture: asks the adapter (reverse RPC
/// <c>debug/captureTest</c>) to launch the repro runner under an ephemeral inline
/// <c>coreclr</c> configuration, arm the all-exceptions filter mid-session and hand back the first
/// exception stop whose stack reaches the workspace. A warm host reaches the stop in about a
/// second and a cold one can take far longer — hence the generous budget on the TypeScript side,
/// not here.
/// </summary>
