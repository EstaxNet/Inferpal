...
internal static class FimContextBuilder
{
    ...
    /// <summary>
    /// Returns true when the editor's native completion dropdown is likely active and FIM
    /// should be suppressed. Editor SDKs expose no reliable API to query the dropdown state
    /// directly, so we use trigger-character detection as a reliable proxy.
    /// </summary>
    public static bool ShouldSuppress(string text, int cursorOffset)
    {
        if (cursorOffset == 0) return false;

        return IntelliSenseTriggers.Contains(text[cursorOffset - 1]);
    }

    /// <summary>Builds the FIM prefix/suffix window around <paramref name="cursorOffset"/>.</summary>
    public static FimContext Build(string text, int cursorOffset)
    {
        var prefix = text[..cursorOffset];
        var suffix = text[cursorOffset..];
