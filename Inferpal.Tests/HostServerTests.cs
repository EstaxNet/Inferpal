...
public partial class HostServerTests
{
    ...
    public async Task Session_SaveLoadDelete_RoundTripsAndRebuildsHistory()
    {
        ...
        try
        {
            ...
            // Host history rebuilt: fresh system prompt + the question and the answer — the history the
            // model had live. The tool bubble stays in the saved transcript (above) and out of the model's
            // history: live, a run's tool results do not outlive the run.
            ...
            Assert.DoesNotContain(history, m => (m.Content ?? string.Empty).Contains("tool output"));
